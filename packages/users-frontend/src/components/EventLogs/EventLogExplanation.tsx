...
export const EventLogExplanation: React.FC<{ record: EventLog }> = ({ record }) => {
  const duration =
    record.duration !== null ? (
      <Tag icon={<ClockCircleOutlined />}>{record.duration} ms</Tag>
    ) : (
      'unknown time '
    );
  ...
  let queryParamsEl: React.ReactNode = (
    <>
      N/A
      <br />
    </>
    ...
    if (queryParams.length) {
      queryParamsEl = (
        <Descriptions
          ...
          size="small"
          bordered={true}
          items={queryParams.map(([key, value], i) => {
            return {
              key: i,
              label: key,
              children:
  ...
  let userEl = record.user ? (
    <>
      User <Tag>{record.user.name}</Tag>which had role{' '}
      <Tag icon={getUserRoleIcon(record.userRole)}>{record.userRole}</Tag>
    </>
  ) : (
    <>
  ...
  return (
    <Card
      title={
        <>
          {getLogTypeTag(record.logType)}captured at{' '}
          <Tag icon={<ClockCircleOutlined />}>{record.createdAt}</Tag>with log level {logLevelEl}
        </>
      }
    >
      {(record.logType === EventLog.logType.AUDIT && (
        <Paragraph>
          {userEl}
          {actionEl} as result of the HTTP request
          <br />
          <pre>
          ...
          Request was done from IP address <Tag>{record.ipAddress}</Tag>
          <br />
      ...
      <Paragraph>
        ...
        The action was handled by service <Tag>{record.service}</Tag>
        {record.serviceId ? (
          <>
            with ID{' '}
            <Text code copyable>
      ...
      {record.duration || record.statusCode ? (
        <Paragraph>
          <Title level={5}>Response</Title>
          The request took {duration}and returned status code {statusCode}
          <br />
        </Paragraph>
      ) : null}
    </Card>
