...
export class QuotesStatisticModule {
  static forRoot(config: { dataSourceName?: string }) {
    return {
      ...
      imports: [ScheduleModule, TypeOrmModule.forFeature([QuotesStatistic], config.dataSourceName)],
      providers: [
        {
          provide: QuotesStatisticService,
          inject: [getDataSourceToken(config.dataSourceName)],
          useFactory: (dataSource: DataSource) => {
            return new QuotesStatisticService(
              dataSource.getRepository(QuotesStatistic),
              dataSource,
            );
          },
        },
      ],
      exports: [QuotesStatisticService],
      controllers: [QuotesStatisticsController],
    };
  }
