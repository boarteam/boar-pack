...
export class ProviderMonitoringService implements OnModuleInit, OnModuleDestroy {
  ...
  private notificationsJournal = new Map<
    TProvider['id'],
    {
      attempts: number;
  ...
  private async getOpenProblematicPeriods(
    providerIds: TProvider['id'][],
  ): Promise<Map<ProvidersProblematicPeriod['providerId'], TOpenProblematicPeriod>> {
  ...
  private async checkProviderActivity() {
    const providers = (await this.fetchProviders()).filter((provider) => provider.threshold); // Skip providers without a threshold;
    ...
    const latestQuotesByProviders =
      await this.quotesStatistic.getLatestQuotesByProvider(providersIds);
  ...
  private async updateProblematicPeriods(
    providers: TProvider[],
    latestQuotesByProviders: Map<TProvider['id'], Date | null>,
  ) {
    const openProblematicPeriods = await this.getOpenProblematicPeriods(
    ...
    providers.forEach((provider: TProvider) => {
      ...
      if (isProblematic && !problematicPeriod) {
        this.logger.warn(
          `Opening problematic period for provider ${provider.name} with threshold ${provider.threshold}`,
        );
  ...
  private async sendNotifications(
    ...
    const telegramNotificationsEnabled = await this.getSetting(
    ...
    providers.forEach((provider: TProvider) => {
      ...
      if (isProblematic) {
        const log =
          diff === null
            ? `Provider ${provider.name} has no quotes`
            : `Provider ${provider.name} has no quotes in the last ${diff} seconds`;
        ...
        if (previousNotification) {
          ...
          if (
            previousNotification.attempts >= this.exponentialBackoff.length ||
            diff < this.exponentialBackoff[previousNotification.attempts]
          ) {
            this.logger.warn(
              `Skipping notification for ${provider.name} due to exponential backoff`,
            );
            return;
          }
        }
