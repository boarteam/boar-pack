...
import {
  TargetsConnectionsStatisticQueryDto,
  UsersConnectionsStatisticQueryDto,
} from './dto/users-connections-statistic-query.dto';
...
export class UsersConnectionsStatisticController {
  constructor(readonly service: UsersConnectionsStatisticService) {}
  ...
  async getTargetsTimeline(
    @Query() query: TargetsConnectionsStatisticQueryDto,
  ): Promise<UsersConnectionsStatisticDto[]> {
    const start = query.startTime ? new Date(query.startTime) : undefined;
    const end = query.endTime ? new Date(query.endTime) : undefined;
    return this.service.getTargetsTimeline({
