...
export class EcnInstrumentsService extends TypeOrmCrudService<EcnInstrument> {
  ...
  private static compareSubscrSchemas = `${
    EcnInstrumentsService.subscriptionJsonObject
  } = (select data from compare_data where subscr.instrument_hash = compare_data.instrumentHash)`;
  ...
  private static connectionsWithEqual = `
    json_objectagg(
      ...
      json_object(
        ...
        'equal', ${EcnInstrumentsService.compareSubscrSchemas}
      )
    )
  `;
  ...
  async getInConnections({
    ...
    const dataQuery = this.repo
      ...
      .innerJoin(EcnSubscrSchema, "subscr", "instrument.instrumentHash = subscr.instrumentHash")
    ...
    if (search) dataQuery.andWhere('instrument.name like :search', { search: `%${search}%` });

    if (compareConnectSchemaId !== undefined) {
      dataQuery.addSelect(EcnInstrumentsService.connectionsWithEqual, "connections")
      dataQuery.addCommonTableExpression(
        this.dataSource
          ...
          // .innerJoin(EcnConnectSchema, "connect", "subscr.connectSchemaId = connect.id")
          .where("subscr.connectSchemaId = :id", { id: compareConnectSchemaId }),
        'compare_data',
      );
      if (showOnlyChanged) {
        dataQuery.having(
          `(count(distinct ${EcnInstrumentsService.compareSubscrSchemas}) > 1 or count(connect.id) <> ${ids.length})`
        );
      }
    } else {
      dataQuery.addSelect(EcnInstrumentsService.connectionsWithoutEqual, "connections");
    ...
    const dataPromise = dataQuery.getRawMany<GetEcnInstrumentsInConnectionsData>();
