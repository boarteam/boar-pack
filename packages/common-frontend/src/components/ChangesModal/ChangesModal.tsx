...
export type TRelationalFields = Map<
  string,
  {
    key: string;
    data: {
...
export function ChangesModal<Entity extends Record<string, any>, ImportRequestParams>({
  ...
  const handleCommitClick = async () => {
    ...
    const modifiedSource = resolvedData && resolvedData.length > 0 ? resolvedData : updated;
  ...
  const tabList = [
    {
      ...
      label: tableData.length ? (
        <Badge size="small" color="blue" count={tableData.length}>
          Changed Values
        </Badge>
      ) : (
        'Changed Values'
      ...
      children: (
        <ChangesTab changedRecordsColumnsConfig={changedRecordsColumnsConfig} updated={tableData} />
      ),
    },
    {
      ...
      label: created.length ? (
        <Badge size="small" color="blue" count={created.length}>
          New Records
        </Badge>
      ) : (
        'New Records'
  ...
  return (
    ...
    >
      <Tabs activeKey={activeTab} onChange={setActiveTab} defaultActiveKey="1" items={tabList} />
    </Modal>
  );
}
