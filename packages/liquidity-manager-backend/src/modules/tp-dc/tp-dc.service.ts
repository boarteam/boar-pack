...
import { MTAttachStreamRequest, MTGetPositionsRequest, MTGetPositionsResult, MTWSMessage } from "./dto/tp-dc.dto";
...
type TTpSocketsConfig = TBaseConfig<MTWSMessage> & {
  // instruments: string[];
}

@Injectable()
export class TpDcService {
  private readonly logger = new Logger(TpDcService.name);
  private readonly VERSION = 10;
  private readonly config = this.configService.config;

  constructor(
    private httpService: HttpService,
    private readonly configService: TpDcConfigService,
  ...
  public getHttpUrl(serverId: number): string {
    ...
    return `http://tp-tst-srv-01:3000/?${params.toString()}`;
  ...
  public getWsUrl(serverId: string | number): string {
    ...
    return `ws://tp-tst-srv-01:3000/stream?${params.toString()}`;
  }
  ...
  public checkStreamConnection({
    url,
    // instruments,
  }: Pick<TTpSocketsConfig, 'url'>): Promise<void> {
