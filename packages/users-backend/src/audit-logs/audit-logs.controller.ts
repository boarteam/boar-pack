...
export class AuditLogsController implements CrudController<AuditLog> {
  ...
  async getMany(@Req() request: Request, @ParsedRequest() req: CrudRequest<TUser>) {
    ...
    if (ability.cannot(Action.Manage, 'all')) {
      const tablesNames = this.service.repo.manager.connection.entityMetadatas
        .filter(
          (meta) =>
            meta.target instanceof Function && ability.can(Action.Manage, meta.target as Subjects),
        )
        .map((meta) => meta.tableName);

      req.parsed.search = {
        $and: [req.parsed.search, { tableName: { $in: tablesNames } }],
      };
    }
